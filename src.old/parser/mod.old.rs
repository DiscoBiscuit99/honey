...
fn parse_statement(tokens: Vec<Token>) -> (SyntaxTree, Vec<Token>) {
    ...
    let _full_stop = tokens
        .next()
        .expect("Failed to parse statement: no full stop?");
...
fn parse_term(tokens: Vec<Token>) -> (SyntaxTree, Vec<Token>) {
    let (factor, tokens) = parse_factor(tokens);
...
fn parse_factor(tokens: Vec<Token>) -> (SyntaxTree, Vec<Token>) {
    let mut tokens = tokens.iter().peekable();

    let (factor, tokens) = if tokens
        .peek()
        .is_some_and(|t| **t == Token::Special(Special::LeftParenthesis))
    {
        // Parse left parenthesis.
        let _l_paren = tokens.next().expect("Failed to parse factor, expected '('");
        ...
        let _r_paren = tokens.next().expect("failed to parse factor, expected ')'");
...
mod tests {
    ...
    fn parse_term() {
        let tokens = vec![
            Token::Special(Special::LeftParenthesis),
            Token::Literal(Literal::Int(314)),
            Token::Special(Special::RightParenthesis),
        ];
        let (tree, _) = super::parse_factor(tokens);
