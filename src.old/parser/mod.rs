...
fn parse_declaration(tokens: &Tokens) -> Result<(SyntaxTree, Tokens), ParsingError> {
    let (keyword, remaining_tokens) = parse_keyword(tokens)?;

    let declaration_keyword = match keyword {
        SyntaxTree::Keyword(keyword_type) => match keyword_type {
            Keyword::DeclarationKeyword(decl_keyword) => decl_keyword,
            _ => Err(ParsingError::ExpectedDeclKeyword {
                found: Some(keyword_type),
            })?,
        },
        _ => Err(ParsingError::ExpectedKeyword {
            found: Some(keyword),
        })?,
    };

    let (identifier, remaining_tokens) = parse_identifier(&remaining_tokens)?;

    let mut tokens_iter = remaining_tokens.iter();

    // Assert that the next token is assignment.
    let next =
        tokens_iter
            .next()
            .ok_or(ParsingError::ExpectedToken(ExpectedToken::Assignment {
                found: None,
            }))?;

    if *next != Token::Assignment {
        Err(ParsingError::ExpectedToken(ExpectedToken::Assignment {
            found: Some(next.clone()),
        }))?;
    }

    // Assert that the next two tokens are the type annotation.
    let next = tokens_iter
        .next()
        .ok_or(ParsingError::ExpectedToken(ExpectedToken::Colon {
            found: None,
        }))?;

    if *next != Token::Colon {
        Err(ParsingError::ExpectedToken(ExpectedToken::Colon {
            found: Some(next.clone()),
        }))?;
    }
...
fn parse_keyword(tokens: &Tokens) -> Result<(SyntaxTree, Tokens), ParsingError> {
    ...
    let next = tokens_iter
        .next()
        .ok_or(ParsingError::ExpectedKeyword { found: None })?;

    // Assert that the next token is the expected token.
    let keyword = if let Token::Keyword(keyword) = next {
        Ok(keyword.clone())
    } else {
        Err(ParsingError::ExpectedToken(ExpectedToken::Keyword {
            found: Some(next.clone()),
        }))
    }?;

    // Wrap it up in a syntax tree.
    let subtree = SyntaxTree::Keyword(keyword);
...
fn parse_identifier(tokens: &Tokens) -> Result<(SyntaxTree, Tokens), ParsingError> {
    ...
    // Assert that the next token exists.
    let next = tokens_iter
        .next()
        .ok_or(ParsingError::ExpectedIdentifier { found: None })?;

    // Assert that the next token is the expected one.
    let identifier = if let Token::Identifier(identifier) = next {
        Ok(identifier.clone())
    } else {
        Err(ParsingError::ExpectedToken(ExpectedToken::Identifier {
            found: Some(next.clone()),
        }))
    }?;

    // Wrap it up in a syntax tree.
    let subtree = SyntaxTree::Identifier(identifier);
